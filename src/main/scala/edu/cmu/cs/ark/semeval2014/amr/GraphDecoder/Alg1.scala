    ...
    def decode(input: Input) : DecoderResult = {
        ...
        for { (node1, index1) <- nodes.zipWithIndex
            ...
            if (relations.count(_ ==label) == 0) {   // relations.count(_ == label) counts the edges that are already in the graph fragments
                ...
                // TODO: Fix below line (need to call features.setupNodes(node1, node2))
                val nodes2 : Array[(Node, Int, Double)] = nodes.zipWithIndex.map(x => (x._1, x._2, graphObj.localScore(index1, x._2, label))).filter(x => x._3 > 0 && x._1.id != node1.id).sortBy(-_._3).take(maxCardinality)
                ...
                // TODO: Fix below line (need to call features.setupNodes(node1, node2))
                val nodes2 : Array[(Node, Int, Double)] = nodes.zipWithIndex.map(x => (x._1, x._2, graphObj.localScore(index1, x._2, label))).filter(x => x._3 > 0 && x._1.id != node1.id && !relationIds.contains(x._1.id)).sortBy(-_._3).take(maxCardinality - relations.count(_ == label))
                for ((node2, index2, weight) <- nodes2) {
                    logger(1, "Adding edge ("+node1.concept+", "+label +", "+node2.concept + ") with weight "+weight.toString)
                    graphObj.addEdge(node1, index1, node2, index2, label, weight)
                }
