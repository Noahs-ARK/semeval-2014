...
\newcommand{\codenote}[1]{\textcolor{PineGreen}{[\emph{Code:} #1]}}
...
The graph-based parser has more sophisticated decoding constraints.
...
\codenote{Look for use of flag LRParser.useHashing, esp LRParser.java:perceptNum() 
  (hashed value before label conjunction),
  and Model.java:coefIdx() (hashed value after label conjunction)}

\noindent
During our experiments, the number of instantiated features, before conjunction with labels, ranged in the tens of millions;
since our implementation
maintained a hash table of feature names to numeric indexes, the
resulting several gigabytes of
memory usage was an issue for conveniently running many simultaneous experiments.
As a simple memory optimization for the \logitedge\ system,
we implemented (multitask) feature hashing
\cite{weinberger_feature_2009},
which randomly assigns features' numeric indexes with a hash function.
Let $F$ be the space of all feature names (before conjunction with the output label),
and let $h$ be a hash function for the final conjunction,
that maps the feature name and the label to one of $B$ bucket values:
$h : (F \times L) \rightarrow \{1, \ldots, B\}$.
Instead of storing model parameters in the usual approach,
as a matrix $\bm\phi$ size $|F| \times |L|$,
under hashing they are stored in a matrix $h(\bm\phi)$ size $B \times |L|$,
where an entry of $h(\bm\phi)$ is defined as
\[
  h(\bm\phi)_{b,l} = \sum_{\substack{\text{feat} \in F \\\text{ s.t. }
h(\text{feat},l)=b}}{\phi_{\text{feat},l}}
\]
We similarly store our feature vectors in a hashed space.
The theory of feature hashing shows that, even with collisions that erroneously force multiple features to use the same weight value,
the final dot products are approximately preserved.
 We used a value of $B$ as large as was convenient for memory usage, $B=10^{8}$,
 which was actually higher than $|F|$ (but still had collisions:
 with $|F|=10e6$ and $B=100e6$, about 9.5\% of features are hashed to a value shared by at least one other features).
%% simulation:
...
%%  average over simulations for last value:
the memory savings were due to
not having to represent the name-to-index map during feature extraction.
