...
There is additional structure in the formalisms which could be
exploited.
\emph{PCEDT} annotations were originally a tree, but underwent a preprocessing step by
...
For example, in 
Figure~\ref{fig:formalisms}
the ``CPR'' arcs incoming to ``flashlights'' and ``alarms'' 
 are a result of this step.
There was originally just one arc pointing to ``and.''
\emph{PCEDT} trees were also mostly deterministic with respect to non-coordination arcs
before this step.
We conjecture that this copying is one reason that determinism constraints were
not helpful for \emph{PCEDT}.
...
Our work has shown the best \nas{what?  best comparing what to what?
  this was not in the paper} results when using one model for predicting whether
...
we found enforcing determinism in the \emph{PAS} formalism gave improvements, while
enforcing other constraints did not.
While our final approach was fairly standard for work in parsing, we
have noted some additional features and constraints which did not
appear to help.
