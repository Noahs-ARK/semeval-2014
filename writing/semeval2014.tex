...
%\noindent
The model treats every ordered pair of token indices $(i, j)$ as an
independent
...
$L_d$ was chosen to be the set of edges that were $> 99.9\%$ deterministic in
the training data\footnote{
For \emph{DM}, $L_d=L \setminus$ \{``\_and\_c,'' ``\_or\_c,'' ``\_then\_c,''
``loc,'' ``mwe,'' ``subord''\}, for \emph{DM}, $L_d = L$, and for \emph{PCEDT},
$L_d=\emptyset$.}.
...
decoder adds for the highest scoring outgoing edge, if its weight is positive.
