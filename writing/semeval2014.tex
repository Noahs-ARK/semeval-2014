...
\usepackage[colorlinks=true,linkcolor=black,citecolor=black,urlcolor=myblue]{hyperref}
...
%Parsing \citep{oepens_broad_2014} includes three different semantic
...
\citep{taskar_max_2003,tsochantaridis_support_2004} that enforces 
...
it did improve \PAS\ by $\approx0.2\%$ absolute $F_1$, but we did not submit \logitedge\ for \PAS.
...
concatenation, difference, inner product, and element-wise multiplication of the two vectors associated with a parent-child edge. We also trained a random forest\footnote{Using \citeposs{RandomForest2002} \emph{randomForest} R implementation.} on the word vectors (using the vector elements as features to predict the observed semantic label); at test time, the predicted hard labels were incorporated as features.
...
to \citet{flanigan-etal:ACL2014}. Almost all semantic dependency graphs are connected (ignoring singletons) in the
